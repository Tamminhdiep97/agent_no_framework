...
ALL_TOOL_SPECS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Get nutritional facts for a food item",
    ...
    },
    {
        ...
        "function": {
            "name": "check_symptom",
            "description": "Provide general info about a medical symptom",
    ...
    def __init__(self):
        my_tool_specs = [
            ...
            if spec["function"]["name"] in {
                "get_top_headlines", "search_news_articles", "get_news_source_info"
            }
        ]
        my_tool_instruction = (
...
class HealthAgent(BaseAgent):
    DESCRIPTION = "Provides nutrition facts, symptom guidance, and local clinic info."

    def __init__(self):
        my_tool_specs = [
            spec for spec in ALL_TOOL_SPECS
            if spec["function"]["name"] in {
                "get_nutrition_info", "check_symptom", "find_local_clinics"
            }
        ]
        my_tool_instruction = (
            "You can look up food nutrition, give general advice on symptoms, and locate nearby clinics."
        )
        system_prompt = make_tool_agent_prompt(
            agent_name=self.__class__.__name__,
            tool_instruction=my_tool_instruction,
            base_template=PROMPT_HEALTH_AGENT_TEMPLATE,
        )
        super().__init__(name="HealthAgent", system_prompt=system_prompt, tools_enabled=True)
        self.my_tool_specs = my_tool_specs
